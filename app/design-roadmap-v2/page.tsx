        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-14 w-full max-w-[1400px] mx-auto">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="bg-white/5 rounded-2xl p-6 sm:p-7 border border-white/10" style={{ borderTopWidth: 4, borderTopColor: C.papaya }}>
              ...
              <ul className="space-y-2.5">
                {['Design system defined but not yet launched', 'Inconsistent voice, tone, and conversational\u00A0structures', 'No app designer — multi-surface is a\u00A0gap', 'No coverage for Credit, Wallet, New\u00A0Bets', 'No research infrastructure or\u00A0tooling'].map((item) => (
...
const slides = [SlideCover, SlideVision, SlideToday, SlideTeamDetail, SlideHiring, SlideStaffAug, SlideRoadmapOverview, SlideNow, SlideResearch, SlideContentDesign, SlideClosingGoal]
...
export default function DesignRoadmapPage() {
  ...
  const { comments, commentMode, setCommentMode, addComment, deleteComment, editComment, flagComment, resolveComment, addReply, deleteReply } = useComments('design-roadmap-v2-comments')
  const { progress: pdfProgress, download: downloadPdf, cancel: cancelPdf } = useSlidePdf('design-roadmap-v2.pdf')
