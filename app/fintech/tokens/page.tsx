...
const sections: { label: string; key: keyof typeof content['en'] }[] = [
                ...
                <div className="rounded-2xl border border-slate/10 overflow-hidden">
                  ...
                  {tokenKeys.map((tokenKey, i) => (
                    ...
                    >
                      ...
                      {/* Values per language */}
                      {languages.map(lang => {
                        const val = (content[lang.code][key] as Record<string, string>)[tokenKey]
                        return (
                          <div key={lang.code} className="px-4 py-3">
                            <p className="text-[13px] text-slate leading-snug">{val}</p>
                          </div>
                        )
                      })}
                    </div>
